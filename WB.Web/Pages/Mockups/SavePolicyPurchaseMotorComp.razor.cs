...
using WB.Web.Extensions;

namespace WB.Web.Pages.Mockups
{
    public partial class SavePolicyPurchaseMotorComp : ComponentBase
    {
        ...
        private string PremiumAccordionClass = "accordion-item custom-accordion-primary";
        private string VehicleAccordionClass = "accordion-item custom-accordion-primary";
        private string RoadSideAccordionClass = "accordion-item";
        private string PaintProtectionAccordionClass = "accordion-item";
        private string PersonalAccidentAccordionClass = "accordion-item";
        private string ReplacementVehicleAccordionClass = "accordion-item";
        private string InsuredPersonAccordionClass = "accordion-item custom-accordion-primary";
        private string InsuredAddressAccordionClass = "accordion-item custom-accordion-primary";
        #endregion
        ...
        public List<PremiumDetail> PremiumDetailsData { get; set; } = new List<PremiumDetail>
    {
        ...
        private void UpdateAccordionColors()
        {
            PremiumAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.PremiumCalculation);
            VehicleAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.VehicleInformation);
            RoadSideAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.AddOnInformation.AddOnRoadsideAssistInformation);
            PaintProtectionAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.AddOnInformation.AddOnPaintProtectionInformation);
            PersonalAccidentAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.AddOnInformation.AddOnPersonalAccidentPlanInformation);
            ReplacementVehicleAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.AddOnInformation.AddOnReplacementVehicleInformation);
            InsuredPersonAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.InsuredPersonInformation);
            InsuredAddressAccordionClass = CssClassRenderer.GetAccordionCssClass(PolicyPurchaseRequest.InsuredAddressInformation);
        }
        #endregion
        ...
        {
            var response = await dialogService.Confirm("Are you sure you would like to submit the request for Quotation Request # 87452?",
                "Confirm",
                new ConfirmOptions() { OkButtonText = "Submit", CancelButtonText = "Cancel" });
            if (response == true)
                navigationManager.NavigateTo("/list-quotations", true);
        }
