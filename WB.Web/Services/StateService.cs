...
public class AppState
{
    ...
    public string LayoutStyles { get; set; } = "detached";          // doublemenu, detached, icon-overlay, icontext-menu, closed-menu, default-menu 
    ...
    public string ThemePrimary { get; set; } = "203, 146, 78";                      // '58, 88, 146', '92, 144, 163', '161, 90, 223', '78, 172, 76', '223, 90, 90'
...
public class StateService
{
    ...
    public async Task colorthemeFn(string val, bool stateClick)
    {
        ...
        if (val == "light")
        {
            _currentState.ThemePrimary = "203, 146, 78";
            await _jsRuntime.InvokeVoidAsync("interop.setCssVariable", "--primary-rgb", _currentState.ThemePrimary);
        ...
        await _jsRuntime.InvokeVoidAsync("interop.addAttributeToHtml", "data-menu-styles", val == "light" ? "color" : val);
        if (stateClick)
        {
            if (val == "light")
            {
                await _jsRuntime.InvokeVoidAsync("interop.addAttributeToHtml", "data-menu-styles", "color");
                await menuColorFn("color");
            }
            else
            {
    ...
    public async Task retrieveFromLocalStorage()
    {
        ...
        string WBmenuposition = _currentState.MenuPosition;
        await menuPositionFn(WBmenuposition);
        string WBheaderposition = _currentState.HeaderPosition;
        await headerPositionFn(WBheaderposition);
        string WBcolortheme = _currentState.ColorTheme;
        await colorthemeFn(WBcolortheme, false);
        string WBbgimg = _currentState.BackgroundImage;
        if (!string.IsNullOrEmpty(WBbgimg))
        {
            await backgroundImageFn(WBbgimg);
        }
        string WBbgcolor = _currentState.ThemeBackground;
        string WBbgcolor1 = _currentState.ThemeBackground1;
        if (!string.IsNullOrEmpty(WBbgcolor))
        {
            await themeBackgroundFn(WBbgcolor, WBbgcolor1,false);
            _currentState.ColorTheme = "dark";
            // _currentState.MenuColor = "dark";
            // _currentState.HeaderColor = "dark";
        }
        string WBMenu = _currentState.MenuColor;
        await menuColorFn(WBMenu);
        string WBHeader = _currentState.HeaderColor;
        await headerColorFn(WBHeader);
        string WBmenuStyles = _currentState.MenuStyles;
        string WBverticalstyles = _currentState.LayoutStyles;
        if (string.IsNullOrEmpty(WBverticalstyles))
        {
            await menuStylesFn(WBmenuStyles);
        }
        else
        {
            await layoutStylesFn(WBverticalstyles);
        }
        string WBprimaryRGB = _currentState.ThemePrimary;
        await themePrimaryFn(WBprimaryRGB);
    }
    public async Task retrieveFromLandingLocalStorage()
    {
        ...
        string direction = await _jsRuntime.InvokeAsync<string>("interop.getLocalStorageItem", "WBdirection") ?? _currentState.Direction;
        await directionFn(direction);
        string WBcolortheme = await _jsRuntime.InvokeAsync<string>("interop.getLocalStorageItem", "WBcolortheme") ?? _currentState.ColorTheme;
        await colorthemeFn(WBcolortheme, false);
        string WBprimaryRGB = await _jsRuntime.InvokeAsync<string>("interop.getLocalStorageItem", "WBprimaryRGB") ?? _currentState.ThemePrimary;
        await themePrimaryFn(WBprimaryRGB);
    }
