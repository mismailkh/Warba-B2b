...
namespace WB.Web.Extensions
{
    public static class CssClassRenderer
    {
        #region Policy Form Accordions
        public static string GetAccordionCssClass(object section)
        {
            if (section == null)
                return "accordion-item custom-accordion-primary";

            if (ValidationHelper.HasValidationErrors(section))
                return "accordion-item custom-accordion-danger";
            else
                return "accordion-item custom-accordion-success";
        }
        #endregion
